...
//			printf("%lf ", centroid[j][i]);
	...
	// Exchange texture
	// Set up Texture window
	unsigned char texture[4][128][128];
	for(int i = 0; i < 128; i++) {
		for(int j = 0; j < 128; j++) {
			texture[0][i][j] = image[i][j];
			texture[1][i][j] = image[i][j+384];
			texture[2][i][j] = image[i+384][j];
			texture[3][i][j] = image[i+384][j+384];
		}
	}
	...
	for(int i = 0; i < 4; i++) {
		for(int j = 0; j < 4; j++) {
			for(int r = 0; r < 128; r++) {
				for(int c = 0; c < 128; c++) {
					if(label[i*128+r][j*128+c] == 0)
						image[i*128+r][j*128+c] = texture[1][r][c];
					if(label[i*128+r][j*128+c] == 1)
						image[i*128+r][j*128+c] = texture[2][r][c];
					if(label[i*128+r][j*128+c] == 2)
						image[i*128+r][j*128+c] = texture[3][r][c];
					if(label[i*128+r][j*128+c] == 3)
						image[i*128+r][j*128+c] = texture[0][r][c];
				}
			}
		}	
	}
...
int main(int argc, char** argv) {
	...
	char write_filename[] = "output/G.raw";
